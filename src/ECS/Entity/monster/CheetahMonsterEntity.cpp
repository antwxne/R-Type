...
#include "CheetahMonsterEntity.hpp"

CheetahMonsterEntity::CheetahMonsterEntity(): IEntityRegister()
{
}

CheetahMonsterEntity::~CheetahMonsterEntity()
{
}

void CheetahMonsterEntity::create(const std::shared_ptr<ComponentManager> &componentManager, const std::shared_ptr<EntityManager> &entityManager)
{
    _entity = entityManager->create();
    std::shared_ptr<sf::Sprite> sprite = std::make_shared<sf::Sprite>();
    int randFirerate = rand()%(7-1 + 1) + 1;
...
{
    return std::shared_ptr<IEntityRegister>(new CheetahMonsterEntity());
}
