...
void BulletEntity::create(ECS &ecs)
{
    ...
    if (_isBulletFriend) {
        ...
        ecs.subToComponent(bullet, PlayerTag{});
    } else {
        ecs.subToComponent(bullet, Rotate{180});
        ecs.subToComponent(bullet, Acceleration{-1, 0});
        ecs.subToComponent(bullet, EnemyTag{});
    }
    ecs.subToComponent(bullet, BulletTag{});
    ...
    ecs.subToComponent(bullet, Hitbox{100, 100}); // TO DO Edit
}